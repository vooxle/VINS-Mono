...
void FeatureTracker::setMask()
{
    ...
    /* 倾向于留下被追踪时间很长的特征点,构造(cnt，pts，id)序列，（追踪次数，当前特征点坐标，id） */
...
void FeatureTracker::readImage(const cv::Mat &_img, double _cur_time)
{
    ...
    if (cur_pts.size() > 0)
    {
        ...
        for (int i = 0; i < int(forw_pts.size()); i++)
            if (status[i] && !inBorder(forw_pts[i]))
                status[i] = 0;
        /* 根据status,把跟踪失败的点剔除 不仅要从当前帧数据forw_pts中剔除，而且还要从cur_un_pts、prev_pts和cur_pts中剔除
         * prev_pts和cur_pts中的特征点是一一对应的,记录特征点id的ids，和记录特征点被跟踪次数的track_cnt也要剔除
         */
    ...
    /* 更新帧、特征点,当下一帧图像到来时，当前帧数据就成为了上一帧发布的数据*/
    prev_img = cur_img;
    prev_pts = cur_pts;
...
/* 采用RANSAC算法，通过寻找3x3的单应性变换矩阵使得符合该变换的点最多的方法，
 * 来滤除不符合要求的角点
 **/
void FeatureTracker::rejectWithF()
{
    /* for the RANSAC algorithm. N≥8 */
    if (forw_pts.size() >= 8)
    {
        /* 遍历所有特征点，转化为归一化相机坐标系 */
        ...
        vector<uchar> status;
        /* 根据两个图像中的对应点计算基本矩阵,需要归一化相机系，z=1 */
